...
- **The information panel reports the card's setup** when `diagnostics: true` — layout mode, display mode, aspect ratio, span, whether animation is on, the Home Assistant version and the window size. Everything that decides how the card sizes itself, so a screenshot answers the questions a report would otherwise need a second round to settle.
