...
### Added
- **A "Copy diagnostics" button** in the status panel, with `diagnostics: true`. It puts the build, the setup, the data source, any fields the source does not provide, and the recent redraw history on the clipboard as plain text — ready to paste into a bug report, with nothing to crop or attach.
- **The information panel reports the card's setup** when `diagnostics: true` — layout mode, display mode, aspect ratio, span, whether animation is on, the Home Assistant version and the window size. Everything that decides how the card sizes itself, so a screenshot answers the questions a report would otherwise need a second round to settle. It also reports how long the *page* has been loaded, which separates a card the browser reloaded wholesale — a discarded tab restored from minimised, say — from a card replaced underneath a page that kept running. The two look identical in a redraw log and lead opposite ways. Page load and element creation are both stamped with a clock time and an age, alongside how long the card showed nothing before its chart arrived — a blank is a duration, and nothing measured one. The redraw list now spans the page rather than one card element, so a rebuild — which Home Assistant does whenever it re-emits the dashboard config, several times an hour on a tab that has been in the background — no longer wipes the history of what happened before it.
- **The information panel lists recent redraws** when `diagnostics: true` — what triggered each one, the size of the space the card was given, and the size it drew, for the last twenty. Requests that were *not* drawn are listed too, and why: nothing had changed, the space was too small, or a burst of resizes was being held off. A card that misbehaves by resizing itself shows those numbers climbing; a healthy one repeats the same pair. Screenshotting the ℹ️ is now enough to report it, with no need for the browser console. The list keeps the first few entries whatever happens after them, so a long reproduction still shows the size the card started at, with the gap stated rather than left as a jump in the timestamps. An element's very first draw is reported as "new element" whatever triggered it, rather than naming whichever of the load path or the resize observer won the race — a card that Home Assistant had rebuilt used to report "resize", which reads as the card resizing itself. Each draw also says whether the chart was reused or rebuilt: a rebuild replaces every element in one frame and cannot animate, which is the difference between a draw that is noticed and one that is not.
- **The card reports which build it is running.** A release shows its tag, so a prerelease reads `2026.8.3-beta2` rather than the `2026.8.3` it shares with the release. Visible under Card version in the ℹ️ panel — so "check you have the new build" is now a question the card can answer.
...
### Fixed
- **A card that Home Assistant rebuilds no longer flashes empty.** A replacement element starts with nothing in it, and even at thirty milliseconds that is a visible gap between two identical charts — most noticeable coming back to a dashboard that had been left in the background, where the card was torn down and redrawn with a blank frame in between. A replacement now inherits the outgoing chart and shows it until its own is ready. Two cards can never be handed each other's chart: the inheritance is keyed on the location and every display option, so only a card that would draw the same picture inherits one. The same covers waking a dashboard that has been asleep: Home Assistant detaches the panel after a few minutes hidden and puts the same cards back, which used to leave them blank for the whole of their redraw before being rebuilt a second later — two blank frames in about a second. The inherited chart occupies exactly the box its replacement will, so nothing appears to change size on the way in.

- **The card appears about five times faster.** It draws an SVG to measured dimensions, so it has to wait for layout before it can draw at all — and it waited a flat 180 ms, which is most of what anyone sees as the card being slow to arrive next to an entity card that renders straight from Home Assistant with nothing to measure. The container turns out to report its final size one millisecond after the first render and hold it unchanged for the next four hundred, so the wait is now evidence rather than a guess: two frames reporting the same size *and* the same configuration, with the old 180 ms kept as the fallback for a container that really is still moving. Measured 200 ms of blank card down to about 35 ms. Following through on that: the size check is now taken synchronously instead of on the next animation frame, and a ten-millisecond sleep that duplicated a guarantee the line above it already made is gone. A rebuilt card goes from nothing on screen to a finished chart in about 23 ms, against 200 ms before any of this.

- **Returning to a backgrounded tab no longer shows two redraws in a row.** The card would notice the tab was visible, find the forecast expired, and animate the new window into place — and Home Assistant would then replace the card element a second later, because its reconnect handler refetches the dashboard config and cards are compared by object identity rather than content. The card now watches `hass.connected`, which flips back to true just before that config arrives, and holds an ordinary redraw for a couple of seconds. Either the element is gone by then and the redraw was never needed, or it survived and the refresh happens two seconds later than it would have. A redraw you asked for — a changed setting — still goes straight through. Reopening a window that had been hidden long enough for Home Assistant to suspend the dashboard did it differently and needed its own fix: the panel is detached and the same elements are put back, so the card drew twice on the way in — once from the load path, once from the resize observer, the second animating — and was then replaced anyway. Those two now coalesce into one draw, and the draw that restores the chart no longer animates, since it is putting back something that was on screen a moment ago. Re-attachment deliberately does *not* wait: the chart is already gone by then, so waiting would only lengthen the blank — which it did, by six seconds, every time entering or leaving edit mode re-parented the card. A card removed from the page also cancels any redraw it had queued, so a deferred one cannot fire on an element that is no longer there and report its container as 0×0.

- **The card appears about a third faster after Home Assistant rebuilds it.** Mounting produces a burst of redraw requests — the load path, the visibility observer, the resize observer's opening callback — and each one restarted the wait before the first draw, so the card sat blank while the delay accumulated. Measured at 293ms to *start* drawing, of which 21ms was the drawing. The wait now runs from the first request rather than the last, which absorbs the same burst without letting it pile up: about 190ms, most of it still deliberate settling time.

- **The card could grow until it filled the screen, in a panel dashboard.** A panel gives the card no height of its own, so the card fell back to the height of its own contents — and then drew the chart to fit the height it had just filled. Each redraw added the small difference between the two, and a resize, which redraws repeatedly, walked the card down the screen a few pixels at a time. The chart is now taken out of that measurement entirely, and where a dashboard supplies no height the card takes one from its aspect ratio, so the size is settled before anything is drawn. ([#46](https://github.com/DTekNO/lovelace-meteogram-card/issues/46))
...
- **The opening animation no longer replays when Home Assistant replaces the card.** It is a page-load flourish, and it earned its keep once — but a card element can be rebuilt without the page reloading, and every rebuild is a first draw for that element. Restoring a browser window left minimised gave a blank card and then a full animated build of a forecast that had not changed and had not even been re-fetched. The page's own first draw still animates.
- **The status panel no longer mixes time zones.** Expires At was local wall-clock time and Last Render was a UTC ISO string, side by side and neither labelled. Everything in the panel is now local.
### Documentation
